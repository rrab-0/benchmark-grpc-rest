...
func Subscriber() *zmq.Socket {
	...
	err = subscriber.Connect("tcp://" + os.Getenv("DEV_ZMQ_SUB_HOST") + ":" + os.Getenv("DEV_ZMQ_SUB_PORT"))
	if err != nil {
		log.Printf("SUB Error: %s\n", err)
	}

	GlobalSubscriber = subscriber
	log.Println("ZMQ Subscriber is up at " + os.Getenv("DEV_ZMQ_SUB_HOST") + ":" + os.Getenv("DEV_ZMQ_SUB_PORT"))
