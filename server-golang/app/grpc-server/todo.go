...
func (ts *todoServer) DeleteTodo(ctx context.Context, in *pbTodo.DeleteTodoRequest) (*pbTodo.DeleteTodoResponse, error) {
	todoId := in.GetId()
	var dbTodo db.Todo
	var todoResponse db.Todo

	if err := db.GlobalConnection.Where("id = ?", todoId).First(&todoResponse).Error; err != nil {
		return nil, err
	}
...
func StartTodo() {
	...
	lis, err := net.Listen("tcp", ":"+os.Getenv("DEV_GRPC_SERVER_PORT"))
	if err != nil {
